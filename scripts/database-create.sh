#!/usr/bin/env bash
# make database-create — create a new database, then bootstrap its ADMIN user by reusing the
# schema-user create flow (USER_ADMIN=1). The admin is a real role (UUID + 12-word passphrase)
# that OWNS the new database. Distinct from `make create-db` (isolated tenant + fixed-password
