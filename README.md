...
Two ways to provision a database, for different needs:

- **`make create-db NEW_DB=… NEW_USER=… NEW_PASSWORD=…`** — an **isolated tenant** database with
  a fixed-password, connection-locked, non-admin owner (see [Provisioning isolated
  databases](#provisioning-isolated-databases)).
- **`make database-create`** — prompts for a new database name, creates it, then bootstraps a
